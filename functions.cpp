...
void menu(std::vector<Polygon>& error, std::vector<ListCheck>& list_d, std::vector<ListCheck>& list_w, std::vector<ListCheck>& list_in, std::vector<ListCheck>& list_out, bool& fl1, bool& fl2, bool& fl3, bool& fl4){
    ...
    while(true) {
        ...
        filename="DRC.txt";//filename = get_filename_from_user(false);
        std::cout << "\nEnter the name of the file with the scheme." << std::endl;
        filename1="cheker.cif";//filename1 = get_filename_from_user(true);
        CIFParser parser;
        if(!parser.CIFParser::parse(filename1)){
            std::cout<<"\nError parsing file!";
            break;
        }
        std::vector<CIFObject> objs = parser.getObjects();
        std::vector<CIFObject> _objs=sort_x(objs);
        ...
        if (choice==1){
            ...
            filename2="write_error.txt";//filename2 = get_filename_from_user(false);
